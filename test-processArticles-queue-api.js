...
async function testPortfolioUsesQueueSlots() {
  const context = buildContext();
  const tabs = [
  ...
  assert.ok(context.captured, 'Portfolio process should be enqueued.');
  assert.strictEqual(context.captured.options.reason, 'portfolio_direct_test');
  assert.deepStrictEqual(context.captured.options.manualTextSources, [
    { id: 'manual-src-1', text: 'portfolio body' }
  ]);
  assert.deepStrictEqual(toPlainJson(context.captured.jobs[0]), {
    kind: 'article_analysis',
    ...
    sourceWindowId: null,
    sourceUrl: 'manual://source',
    chatUrl: 'https://portfolio.example',
    promptChainSnapshot: ['p1'],
    queueBatchId: '',
    manualPdfBatchId: '',
    manualPdfProviderId: ''
...
async function main() {
  ...
  await testPortfolioUsesQueueSlots();
