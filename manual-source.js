  ...
  chrome.runtime.sendMessage(payload, (response) => {
    ...
    if (remoteRequested) {
      const runnerLabel = typeof response?.runner?.runnerName === 'string' && response.runner.runnerName.trim()
        ...
        : (typeof response?.runner?.runnerId === 'string' && response.runner.runnerId.trim()
          ? response.runner.runnerId.trim()
          : 'runner');
      const jobId = typeof response?.job?.jobId === 'string' && response.job.jobId.trim()
        ? response.job.jobId.trim()
        : 'n/a';
      submitBtn.textContent = 'Wyslano';
      setProviderStatus(`Zdalna analiza wyslana do ${runnerLabel}. Job=${jobId}.`, 'success');
    } else {
